...
//		setDescription("请选择数据库连接,模板文件位于："+XmlObjectCrud.getFtlPath());
	...
	public void createControl(Composite parent) {
		...
		connList.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if(connList.getSelectionIndex()==-1)return;
				//更新表列表
		...
		tblclmnNewColumn_0.setWidth(80);
		tblclmnNewColumn_0.setText("类型");
	...
	private void setDescrible(DataSource ds){
		StringBuilder sb=new StringBuilder();
		sb.append("数据库类型:").append(ds.getDataBaseType()).append("\t\t");
		sb.append("数据库名:").append(ds.getDbName()).append("\t\t");
		sb.append("数据库驱动:").append(ds.getDriver()).append("\t\t");
		sb.append("数据库地址:").append(ds.getUrl()).append("\t\t");
		sb.append("用户名:").append(ds.getUser()).append("\t\t");
		sb.append("密码:").append(ds.getPassword()).append("\t\t");
	...
	protected DataBindingContext initDataBindings() {
		...
		IObservableMap[] observeMaps = BeansObservables.observeMaps(listContentProvider.getKnownElements(), TableModel.class, new String[]{"name", "type","comment"});
