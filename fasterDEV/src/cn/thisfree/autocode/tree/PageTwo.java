...
public class PageTwo extends WizardPage {
	...
	private Text templateLocation;//模板文件路径 add by huangxiaolong@2015/09/13
	...
	public void createControl(Composite parent) {
		...
		Label lblNewLabel_0 = new Label(container, SWT.NONE);
		lblNewLabel_0.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblNewLabel_0.setText("模板文件夹");
		templateLocation = new Text(container, SWT.BORDER | SWT.READ_ONLY);
		templateLocation.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		
		Button tempBtn = new Button(container, SWT.NONE);
		tempBtn.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				templateLocation.setText(srcPath);
			}
		});
		tempBtn.setText("选择");
	...
	public boolean checkInput(){
		...
		if(StringUtils.isBlank(templateLocation.getText())){
			setMessage("请选择模板文件夹", IMessageProvider.ERROR);
			return false;
		}
		...
		if(StringUtils.isBlank(txtFunctionNameEn.getText())){
			setMessage("请输入功能英文名(EN)", IMessageProvider.ERROR);
			return false;
		}
		
		if(StringUtils.isBlank(txtFunctionNameCn.getText())){
			setMessage("请输入功能的中文名(CN)", IMessageProvider.ERROR);
			return false;
		}
	...
	protected DataBindingContext initDataBindings() {
		...
		//
		IObservableValue templateLocationObserveTextObserveWidget = SWTObservables.observeText(templateLocation, SWT.Modify);
		IObservableValue templateLocationObserveValue = PojoObservables.observeValue(tableConfig, "templateLocation");
		bindingContext.bindValue(templateLocationObserveTextObserveWidget, templateLocationObserveValue, null, null);
		//
